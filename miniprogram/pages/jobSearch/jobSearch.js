...
Page({
  ...
  onSearch: function(e){
      ...
      for(var i=0;i<list.length;i++){
          for(var j=0;j<list[i].contains.length;j++){
              if(list[i].contains[j].name.match(val)!=null&&val!=""){
                var res=[{name:"",describe:""}]
                  res[0].name=list[i].contains[j].name;
                  res[0].describe=list[i].contains[j].describe;
                  arr.push(res[0]);
                  this.setData({
                    res:arr,
                  })
